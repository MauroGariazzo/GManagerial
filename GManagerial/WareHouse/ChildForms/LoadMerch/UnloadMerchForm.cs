...
{
    public partial class UnloadMerchForm : Form
    {
        ...
        public UnloadMerchForm(Warehouse selectedWareHouse)
        {
            InitializeComponent();
            ...
            this._tempMovement = new Movement() { WareHouseFK = _selectedWareHouse.ID, MovementType = "Scarico" };
        }
        ...
        {
            if (!_warehouseproducts.Keys.Contains((warehouseproduct.ID, warehouseproduct.SupplierProps.ID)) && _isNewEditDelete.Equals(IsNewEditCopyDeleteEnum.New))
            {
                _warehouseproducts.Add((warehouseproduct.ID, warehouseproduct.SupplierProps.ID), warehouseproduct);
                int rowIndex = UnloadMerchDGV.Rows.Add();
                UnloadMerchDGV.Rows[rowIndex].Cells[0].Value = warehouseproduct.ID;
                UnloadMerchDGV.Rows[rowIndex].Cells[1].Value = warehouseproduct.ProductName;
                UnloadMerchDGV.Rows[rowIndex].Cells[2].Value = warehouseproduct.SupplierProps.ID;
                UnloadMerchDGV.Rows[rowIndex].Cells[3].Value = warehouseproduct.SupplierProps.SupplierName;
                UnloadMerchDGV.Rows[rowIndex].Cells[4].Value = _selectedWareHouse.ID;
                UnloadMerchDGV.Rows[rowIndex].Cells[5].Value = _selectedWareHouse.Warehouse_Name;
                UnloadMerchDGV.Rows[rowIndex].Cells[6].Value = warehouseproduct.Stock;
            }
        ...
        {
            foreach (DataGridViewRow row in UnloadMerchDGV.Rows) //??
            {
                if (isNewEditDelete.Equals(IsNewEditCopyDeleteEnum.New))
                {
                    if (Convert.ToInt32(row.Cells[0].Value) == warehouseproduct.ID &&
                        Convert.ToInt32(row.Cells[2].Value) == warehouseproduct.SupplierProps.ID)
                    {
                        ...
                        _warehouseproducts[(warehouseproduct.ID, warehouseproduct.SupplierProps.ID)].Stock = stock;
                    }
                }
            }
            ...
            {
                _warehouseproducts.Remove((Convert.ToInt32(UnloadMerchDGV.SelectedRows[0].Cells[0].Value), Convert.ToInt32(UnloadMerchDGV.SelectedRows[0].Cells[2].Value)));

                if (!_warehouseproducts.Keys.Contains((warehouseproduct.ID, warehouseproduct.SupplierProps.ID)))
                {
                    _warehouseproducts.Add((warehouseproduct.ID, warehouseproduct.SupplierProps.ID), warehouseproduct);

                    UnloadMerchDGV.SelectedRows[0].Cells[2].Value = warehouseproduct.SupplierProps.ID;
                    UnloadMerchDGV.SelectedRows[0].Cells[3].Value = warehouseproduct.SupplierProps.SupplierName;

                    UnloadMerchDGV.SelectedRows[0].Cells[5].Value = _selectedWareHouse.Warehouse_Name;
                    UnloadMerchDGV.SelectedRows[0].Cells[6].Value = warehouseproduct.Stock;
                }

                else
                {
                    _warehouseproducts[(warehouseproduct.ID, warehouseproduct.SupplierProps.ID)].Stock += warehouseproduct.Stock;

                    UnloadMerchDGV.Rows.Clear();
        ...
        {
            if (UnloadMerchDGV.Rows.Count > 0)
            {
                ...
                MessageBox.Show("Scarico effettuato", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                MessageBox.Show("Inserisci prima dei prodotti da scaricare", "Attenzione", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        ...
        private void InsertDetailMovementAndProductsInWareHouseDB(int movement_id) //inserisco i dettagli del movimento e i prodotti al magazzino
        {
            foreach (WareHouseProduct wareHouseProduct in _warehouseproducts.Values)
            {
                ...
                if (_daoWareHouseProduct.IsTheProductAlreadyInserted(wareHouseProduct).Equals(-1))
                {
                    _daoWareHouseProduct.Insert(wareHouseProduct);
                }
                else
                {
                    _daoWareHouseProduct.Update(wareHouseProduct, WarehouseProductQueries.UNLOADPRODUCTS);
                }
            }
        }
        ...
        {
            if (UnloadMerchDGV.SelectedRows.Count > 0)
            {
                _isNewEditDelete = IsNewEditCopyDeleteEnum.Edit;
                DataGridViewRow row = UnloadMerchDGV.SelectedRows[0];
        ...
        private void RemoveProduct_Click(object sender, EventArgs e)
        {
            ...
            if (UnloadMerchDGV.SelectedRows.Count.Equals(1))
            {
                ...
                {
                    _warehouseproducts.Remove((Convert.ToInt32(UnloadMerchDGV.SelectedRows[0].Cells[0].Value), Convert.ToInt32(UnloadMerchDGV.SelectedRows[0].Cells[2].Value)));
                    UnloadMerchDGV.Rows.Remove(UnloadMerchDGV.SelectedRows[0]);
                }
            }
        }


        private void UnloadMerchDGV_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            int row = e.RowIndex;
            int cellValue = Convert.ToInt32(UnloadMerchDGV.Rows[row].Cells[6].Value);
            WareHouseProduct warehouseProduct = _warehouseproducts[(Convert.ToInt32(UnloadMerchDGV.SelectedRows[0].Cells[0].Value), Convert.ToInt32(UnloadMerchDGV.SelectedRows[0].Cells[2].Value))];

            if (cellValue != warehouseProduct.Stock)
            {
                if (cellValue > 0)
                {
                    _warehouseproducts[(Convert.ToInt32(UnloadMerchDGV.SelectedRows[0].Cells[0].Value), Convert.ToInt32(UnloadMerchDGV.SelectedRows[0].Cells[2].Value))].Stock = cellValue;
                }
                ...
                else
                {
                    ...
                    UnloadMerchDGV.Rows[row].Cells[6].Value = _warehouseproducts[(Convert.ToInt32(UnloadMerchDGV.SelectedRows[0].Cells[0].Value), Convert.ToInt32(UnloadMerchDGV.SelectedRows[0].Cells[2].Value))].Stock;
                }
            }
        }

        private void UnloadMerchDGV_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            e.Control.KeyPress += new KeyPressEventHandler(UnloadMerchDGV_KeyPress);
        }

        private void UnloadMerchDGV_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (UnloadMerchDGV.SelectedRows[0].Index != -1)
            {
