...
{
    //public delegate void OnformClosing(object sender, FormClosingEventArgs e);
    internal partial class NewOrderForm : Form
    {
        private int _currentRowIndex = 0;
        private OrderDocumentPrint _orderDocumentPrint;
        ...
        private DAOOrder _daoOrder;
        private DAOOrderProduct _daoOrderProduct;
        private OnClosing _onClosing;
        private CloseForm _closeForm;
        private decimal _totalVat;

        public NewOrderForm(Supplier supplier, Warehouse warehouse, OnClosing onClosing, CloseForm closeForm)
        {            
            InitializeComponent();      
             _dbConnector = new DBConnector("Data Source=DESKTOP-TH1C0HD;Initial Catalog=Gmanagerial;Integrated Security=True");
             _selectedWareHouse = warehouse;
             _supplier = supplier;
             _warehouseproducts = new Dictionary<int, WareHouseProduct>();
             _vatValues = new Dictionary<int, string>();
             _totalNet = 0.00m;
             _tempMovement = new Movement() { WareHouseFK = _selectedWareHouse.ID, MovementType = "Carico" };
             _daoMovement = new DAOMovement(_dbConnector);
             _daoDetailsMovement = new DAODetailsMovement(_dbConnector);
             _daoWareHouseProduct = new DAOWarehouseProduct(_dbConnector);
             _orderDocumentPrint = new OrderDocumentPrint(_currentRowIndex, _supplier, OrderDataGridView, TotalNetValue, TotalOrderLbl, VatValueLbl);
             _daoAttachment = new DAOAttachments(_dbConnector, AttachmentQuery.INSERT_ATTACHMENT, string.Empty); //delete attachment non servirebbe nemmeno in quanto non
            //dobbiamo avere la possibilità di cancellare gli allegati da questo form
             _daoObjectAttachments = new DAOObjectAttachments(_dbConnector, string.Empty, AttachmentQuery.INSERT_WH_MOVEMENT,string.Empty);
             _onClosing = onClosing;
             _closeForm = closeForm;
             //_order = new Order();
             _daoOrder = new DAOOrder(_dbConnector);
            _daoOrderProduct= new DAOOrderProduct(_dbConnector);
        }
        ...
        {
            if (!_warehouseproducts.Keys.Contains(warehouseproduct.ID))
            {
                _warehouseproducts.Add((warehouseproduct.ID), warehouseproduct);
                int rowIndex = OrderDataGridView.Rows.Add();
                OrderDataGridView.Rows[rowIndex].Cells[0].Value = warehouseproduct.ID;
                OrderDataGridView.Rows[rowIndex].Cells[1].Value = warehouseproduct.ProductName;
                OrderDataGridView.Rows[rowIndex].Cells[2].Value = warehouseproduct.Description;
                ...
                CalculateVat();
                 ...
                 _totalNet += amount;
                TotalNetValue.Text = $"{Convert.ToString( _totalNet)}€";
                TotalRecalculate();
            }
        ...
        private void CalculateVat()
        {
            ...
            foreach (KeyValuePair<int, string> couple in _vatValues)
            {
                ...
                _totalVat += (vat / 100) * amount;
            }
            _totalVat = Math.Round(_totalVat, 2);

            VatValueLbl.Text = $"{_totalVat.ToString()}€";
        }
        ...
        private void ValidatePrice(string cellValue, int row)
        {
            ...
            {
                if ( !(cellValue is null) && !(string.IsNullOrWhiteSpace(cellValue)))
                {
        ...
        private void PrintBtn_Click(object sender, EventArgs e)
        {
            ...
            printDocument.PrintPage += new PrintPageEventHandler(_orderDocumentPrint.PrintPageHandler);
        ...
        private void LoadMerchandise_Click(object sender, EventArgs e)
        {
            if (OrderDataGridView.Rows.Count > 0)
            {
                DialogResult result = MessageBox.Show( 
                                      "Proseguendo verranno aggiornate le giacenze. \n Vuoi andare avanti?",
                                      "Informazione",
        ...
        {
            DialogResult dialog = MessageBox.Show("Vuoi salvare la stampa generata?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        {            
            decimal value = Convert.ToDecimal(TotalOrderLbl.Text.Substring(0, TotalOrderLbl.Text.Length-1));
            if (OrderDataGridView.Rows.Count > 0 && value > 0)
            {
                DialogResult dialogResult = MessageBox.Show("Vuoi salvare il documento?", "Info", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dialogResult == DialogResult.Yes) 
                { 
                    Order order = CreateOrderDocument();
                    CreateOrderProduct(order);
            ...
            {
                MessageBox.Show("Il prezzo dev'essere maggiore di zero", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
     
        private Order CreateOrderDocument()
        {
            Order order = new Order();
            order.CreationDate = DateTime.Now;
            order.Supplier = _supplier;
            order.TotalDocumentAmount = _totalNet;
            order.TaxAmount = _totalVat;
            order.TotalDocumentAmountWithTax = _totalNet + _totalVat;
            MoveProductsToOrder(order); //aggiungere prodotti del datagridview all'ordine

            order.Id = _daoOrder.Insert(order); //ottenimento id dell'ordine
            return order;
        }

        private void CreateOrderProduct(Order order)
        {
            foreach(OrderProduct product in order.Products)
            {
                int idOrderProduct = _daoOrderProduct.Insert(product);
                //product.ID = idOrderProduct;
            }
        }

        private void CreateOrderDetails(Order order)
        {
            DAOOrderDetails daoOrderDetails = new DAOOrderDetails(_dbConnector);
            foreach (OrderProduct product in order.Products)
            {
                OrderDetails orderDetails = new OrderDetails(order.Id, product.ID);
                daoOrderDetails.Insert(orderDetails);
            }
        }

        private void MoveProductsToOrder(Order order)
        {
            ...
            { //try
                ...
                product.Description = Convert.ToString(row.Cells[2].Value);
                product.Quantity = Convert.ToInt32(row.Cells[3].Value);

                try 
                {
                    product.Price = Convert.ToDecimal(row.Cells[4].Value);
                }
                catch(ArgumentOutOfRangeException ex)
                {
                    MessageBox.Show(ex.Message.Substring(69, 39), "Attenzione",MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    order.Products = new List<OrderProduct>();
                    return;
                }

                product.Discount = Convert.ToDecimal(row.Cells[5].Value);

                if (row.Cells[6].Value.Equals(string.Empty)) 
                {
                    product.Tax = string.Empty;
                }
                else
                {
                    product.Tax = Convert.ToString(row.Cells[6].Value);
                }
            
                try
                {
                    string amount = row.Cells[7].Value.ToString();
                    int euroSymbol = amount.IndexOf("€");
                    product.Amount = Convert.ToDecimal(amount.Substring(0, euroSymbol));
                }
                catch (ArgumentOutOfRangeException ex)
                {
                    MessageBox.Show(ex.Message.Substring(69, 39), "Attenzione", MessageBoxButtons.OK, MessageBoxIcon.Warning);
